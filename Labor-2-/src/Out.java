...
<p>Extended with simple rounding functions by Tobias Hagen, Berufsakademie Loerrach
...
/**
 * rounds a double to a defined number of decimals
...
/**
 * rounds a float to a defined number of decimals
...
public static float round(float f, int n){
	if (n>=0){
		double  factor = Math.pow(10, n);
		return ((Math.round(f*(float)factor))/(float)factor);
	}else{ 
		return f;
	}
}
